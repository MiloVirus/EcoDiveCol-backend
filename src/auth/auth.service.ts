...
export class AuthService {
    ...
    async signIn(signInDto: SignInDto): Promise<{email:string, msg:string, token: string}>
    {
        ...
        const payload = {sub: user.user_id, first_name: user.first_name, last_name: user.last_name, email: user.email, rol: user.rol, ciudad: user.city, padi_naui_id: user.PADI_NAUI_ID}
